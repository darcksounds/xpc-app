...
function ProductCard({ product }) {
  ...
  return (
    ...
    >
      <div style={{
        height: '250px', background: theme.input,
      ...
      <div style={{ padding: '14px' }}>
        ...
        <div style={{ fontSize: '13px', color: theme.muted, marginBottom: '10px', lineHeight: '1.4' }}>
          {product.name}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontSize: '20px', fontWeight: '700', color: theme.text }}>
            {product.price} <span style={{ fontSize: '12px', fontWeight: '400', color: theme.muted }}>₾</span>
          </div>
          {product.inStock && (
            <button
